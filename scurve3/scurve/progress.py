...
class Progress(Inplace):
    ...
    def tick(self, val):
        ...
        run_secs = t.days * 86400 + t.seconds + t.microseconds / 1000000.0
        if pp == 0:
            eta = "?:??:??"
        else:
            to_go = run_secs * (1 - pp) / pp
            eta = datetime.timedelta(seconds=int(to_go))
        if pp > self.prev:
            self.prev = pp
            done_string = self.done * progress
            resuming_string = self.todo * (self.width - progress)
            s = "%s%s%s%s%s %s" % (
                self.bookend,
                done_string,
                self.current,
...
class Dummy:
    ...
    def restore_term(self, *args, **kwargs):
        pass
